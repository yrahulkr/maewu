package com.crawljax.examples.splittypie;
...
public class SplittyPieCrawlingRules {
	...
	public static void setCrawlingRules(CrawljaxConfigurationBuilder builder) {
		...
		builder.crawlRules().waitAfterReloadUrl(SplittyPieRunner.WAIT_TIME_AFTER_RELOAD, TimeUnit.MILLISECONDS);
		builder.crawlRules().waitAfterEvent(SplittyPieRunner.WAIT_TIME_AFTER_EVENT, TimeUnit.MILLISECONDS);

		/* set browser. */
		builder.setBrowserConfig(new BrowserConfiguration(BrowserType.CHROME, 1));

		/* input data. */
		builder.crawlRules().setInputSpec(SplittyPieCrawlingRules.splittyPieSpecification());
