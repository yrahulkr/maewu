...
public class CollabtiveForms {
	...
	static void login(InputSpecification inputAddressBook) {
		...
		FormInput password = loginForm.inputField(InputType.TEXT, new Identification(How.id, "pass"));
		password.inputValues("admin");

		inputAddressBook.setValuesInForm(loginForm).beforeClickElement("button").withAttribute("title", "Login");
	}
