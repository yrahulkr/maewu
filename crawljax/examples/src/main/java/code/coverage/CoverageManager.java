    ...
    public Object getCoverageObject(WebDriver driver){
        ...
        while(coverage == null){
        	if(retries>max_retries) {
        		System.out.println("Could not find coverage even after max tries " + retries);
        		return null;
        	}
        	try {
				Thread.sleep(1000);
        	...
        	coverage = javascriptExecutor.executeScript("return window.__coverage__;");
        	retries+=1;
        }
        System.out.println("Found coverage in " + retries);
        return coverage;
    }
    ...
    public void sendCoverageObjectToExpressServer(Object coverage){
        try {
            ...
            HttpRequest request = requestFactory.buildPostRequest(
                    new GenericUrl("http://localhost:7005"
