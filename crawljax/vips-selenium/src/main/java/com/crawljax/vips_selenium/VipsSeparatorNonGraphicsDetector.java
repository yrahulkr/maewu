...
public class VipsSeparatorNonGraphicsDetector implements VipsSeparatorDetector {
	...
	private void ruleFour(Separator separator)
	{
		...
		for (Node top : topAdjacentElements)
		{
			for (Node bottom : bottomAdjacentElements)
			{
				int diff = Math.abs(Integer.parseInt(DomUtils.getCssProperty(top, "fontsize")) - Integer.parseInt(DomUtils.getCssProperty(bottom, "fontsize")));
				if (diff != 0)
				{
					separator.weight += 2;
					weightIncreased = true;
		...
		for (Node top : topAdjacentElements)
		{
			for (Node bottom : bottomAdjacentElements)
			{
				if (Integer.parseInt(DomUtils.getCssProperty(top, "fontsize")) < Integer.parseInt(DomUtils.getCssProperty(bottom, "fontsize")))
				{
					separator.weight += 2;
					weightIncreased = true;
					break;
	...
	private void ruleFive(Separator separator)
	{
		...
		for (Node top : topAdjacentElements)
		{
			for (Node bottom : bottomAdjacentElements)
			{
				if (DomUtils.isTextBox(top)//top.getBox() instanceof TextBox 
						&&
