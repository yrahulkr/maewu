...
public class VisualStructure {
	...
	public void updateToNormalizedDoC()
	{
		...
		if (_doC == 12)
		{
			...
			{
				if (DomUtils.getDoC(nestedBlock) < _doC)
					_doC = DomUtils.getDoC(nestedBlock);
			}
		}
	...
	public int containImg()
	{
		...
		{
			_containImg += DomUtils.getImageCount(vipsBlock);
		}
	...
	public int containP()
	{
		...
		for (Node vipsBlock : _nestedBlocks)
		{
			_containP += DomUtils.getPCount(vipsBlock);//vipsBlock.containP();
		}
	...
	public boolean containTable()
	{
		...
		for (Node vipsBlock : _nestedBlocks)
		{
			if (DomUtils.containsTable(vipsBlock))
	...
	public boolean isImg()
	{
		...
		return DomUtils.checkIsImg(_nestedBlocks.get(0));//.isImg();
	}
	...
	public int getTextLength()
	{
		...
		for (Node vipsBlock : _nestedBlocks)
		{
			_textLength += DomUtils.getTextLength(vipsBlock);//vipsBlock.getTextLength();
		}
	...
	public int getLinkTextLength()
	{
		...
		for (Node vipsBlock : _nestedBlocks)
		{
			_linkTextLength += DomUtils.getLinkTextLength(vipsBlock);//vipsBlock.getLinkTextLength();
		}
	...
	public int getFontSize()
	{
		if (_nestedBlocks.size() > 0)
			return (DomUtils.getFontSize(_nestedBlocks.get(0)));//_nestedBlocks.get(0).getFontSize();
		else
			return -1;
	}
	...
	{
		if (_nestedBlocks.size() > 0)
			return DomUtils.getCssProperty(_nestedBlocks.get(0), "fontweight");//_nestedBlocks.get(0).getFontWeight();
		else
			return "undef";
	}
	...
	{
		if (_nestedBlocks.size() > 0)
			return DomUtils.getCssProperty(_nestedBlocks.get(0), "bgcolor");
