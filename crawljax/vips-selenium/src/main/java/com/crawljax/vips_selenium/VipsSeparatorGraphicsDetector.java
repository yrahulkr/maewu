...
public class VipsSeparatorGraphicsDetector extends JPanel implements VipsSeparatorDetector {
	 ...
	 */
	public VipsSeparatorGraphicsDetector(BufferedImage screenshot) {
		this._image = screenshot;
...
//				elementBox.getContentHeight());
		...
		org.openqa.selenium.Rectangle rect1 = DomUtils.getRectangle(vipsBlock);
		Rectangle rect = new Rectangle(rect1.x,
				rect1.y, rect1.width,
				rect1.height);
...
//		_pool.fill(rect);
	...
	{
		if (DomUtils.isVisualBlock(vipsBlock))//vipsBlock.isVisualBlock())
		{
...
//		_pool.fillRect(0, 0, _image.getWidth(), _image.getHeight());
	...
	private void findVerticalSeparators()
	{
		for (Node vipsBlock : _visualBlocks)
		{
			...
			org.openqa.selenium.Rectangle rect = DomUtils.getRectangle(vipsBlock);
	...
	private void ruleThree(Separator separator, boolean horizontal)
	{
		...
		for (Node top : topAdjacentElements)
		{
			for (Node bottom : bottomAdjacentElements)
			{
				...
				if (!DomUtils.getCssProperty(top, "bgcolor").equalsIgnoreCase(DomUtils.getCssProperty(bottom, "bgcolor")))
				{
					separator.weight += 2;
	...
	private void ruleFour(Separator separator)
	{
		...
		for (Node top : topAdjacentElements)
		{
			for (Node bottom : bottomAdjacentElements)
			{
				int diff = Math.abs(DomUtils.getFontSize(top) - DomUtils.getFontSize(bottom));
				if (diff != 0)
				{
					separator.weight += 2;
					weightIncreased = true;
				...
				{
					if (!DomUtils.getCssProperty(top, "fontWeight").equalsIgnoreCase(DomUtils.getCssProperty(bottom, "fontWeight")))
					{
						separator.weight += 2;
					}
				}
		...
		for (Node top : topAdjacentElements)
		{
			for (Node bottom : bottomAdjacentElements)
			{
				if (DomUtils.getFontSize(top) < DomUtils.getFontSize(bottom))
				{
					separator.weight += 2;
					weightIncreased = true;
					break;
	...
	private void ruleFive(Separator separator)
	{
		...
		for (Node top : topAdjacentElements)
		{
			for (Node bottom : bottomAdjacentElements)
			{
				if (DomUtils.isTextBox(top) &&
						DomUtils.isTextBox(bottom))
				{
					separator.weight -= 2;
