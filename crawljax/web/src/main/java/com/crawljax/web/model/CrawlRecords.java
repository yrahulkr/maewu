...
public class CrawlRecords {
	...
	/**
	 * @return the most recent successful crawl
	 */
	public CrawlRecord getCurrentCrawlRecord() {
		int currentCrawlID = -1;
		for(CrawlRecord crawlRecord : crawlList) {
			if(crawlRecord.getCrawlStatus() == CrawlRecord.CrawlStatusType.success
					&& crawlRecord.getId() > currentCrawlID) {
				currentCrawlID = crawlRecord.getId();
			}
		}
		if(currentCrawlID == -1) return null;
		return findByID(currentCrawlID);
	}
