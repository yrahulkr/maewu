...
/**
 * Compile a String or other {@link CharSequence}, returning a Java {@link Class} instance that may
 * be instantiated. This class is a Facade around {@link JavaCompiler} for a narrower use case, but
 * a bit easier to use. To compile a String containing source for a Java class which implements
 * MyInterface:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * The source can be in a String, {@link StringBuffer}, or your own class which implements
 * {@link CharSequence}. If you implement your own, it must be thread safe (preferably, immutable.)
 * (https://github.com/oldratlee/jcompiler)
 *
...
public class CharSequenceCompiler<T> {
	 ...
	 *
	 * @param loader
	 *            the application ClassLoader. The compiler will look through to this // class
	 *            loader for dependent classes
	 * @param options
	...
	/**
	 * Compile Java source in <var>javaSource</var> and return the resulting class. Thread safety:
	 * this method is thread safe if the <var>javaSource</var> and <var>diagnosticsList</var> are
	 * isolated to this thread.
	 *
	 * @param qualifiedClassName
	 *            The fully qualified class name.
	 * @param javaSource
	 *            Complete java source, including a package statement and a class, interface, or
	 *            annotation declaration.
	 * @param diagnosticsList
	 *            Any diagnostics generated by compiling the source are added to this collector.
	 * @param types
	 *            zero or more Class objects representing classes or interfaces that the resulting
	 *            class must be assignable (castable) to.
	 * @return a Class which is generated by compiling the source
	 * @throws CharSequenceCompilerException
	 *             if the source cannot be compiled - for example, if it contains syntax or semantic
	 *             errors or if dependent classes cannot be found.
	 ...
	 *
	 * @param classes
	 *            A Map whose keys are qualified class names and whose values are the Java source
	 *            strings containing the definition of the class. A map value may be null,
	 *            indicating that compiled class is expected, although no source exists for it (it
	 *            may be a non-public class contained in one of the other strings.)
	 * @param diagnosticsList
	 *            Any diagnostics generated by compiling the source are added to this list.
	 * @return A mapping of qualified class names to their corresponding classes. The map has the
	 *         same keys as the input <var>classes</var>; the values are the corresponding Class
	 *         objects.
	 * @throws CharSequenceCompilerException
	 ...
	 * Use this method if you need access to additional classes compiled by
	 * {@link #compile(String, CharSequence, DiagnosticCollector, Class...) compile()}, for example
	 * if the primary class contained nested classes or additional non-public classes.
	 *
	 * @param qualifiedClassName
	...
	/**
	 * Check that the <var>newClass</var> is a subtype of all the type parameters and throw a
	 * ClassCastException if not.
	 *
	 * @param types
	 *            zero of more classes or interfaces that the <var>newClass</var> must be castable
	 *            to.
...
/**
 * A JavaFileManager which manages Java source and classes. This FileManager delegates to the
 * JavaFileManager and the ClassLoaderImpl provided in the constructor. The sources are all in
 * memory CharSequence instances and the classes are all in memory byte arrays.
 */
final class FileManagerImpl extends ForwardingJavaFileManager<JavaFileManager> {
	...
	/**
	 * Construct a new FileManager which forwards to the <var>fileManager</var> for source and to
	 * the <var>classLoader</var> for classes
	 *
	 * @param fileManager
	 *            another FileManager that this instance delegates to for additional source.
	 * @param classLoader
	 *            a ClassLoader which contains dependent classes that the compiled classes will
	 *            require when compiling them.
	 */
	...
	/**
	 * For a given file <var>location</var>, return a FileObject from which the compiler can obtain
	 * source or byte code.
	 *
	 * @param location
...
/**
 * A JavaFileObject which contains either the source text or the compiler generated class. This
 * class is used in two cases.
 * <ol>
 ...
 * <li>The Java compiler also creates instances (indirectly through the FileManagerImplFileManager)
 * when it wants to create a JavaFileObject for the .class output. This uses the
 * {@link JavaFileObjectImpl#JavaFileObjectImpl(String, JavaFileObject.Kind)} constructor.
 * </ol>
 * This class does not attempt to reuse instances (there does not seem to be a need, as it would
 * require adding a Map for the purpose, and this would also prevent garbage collection of class
 * byte code.)
 */
