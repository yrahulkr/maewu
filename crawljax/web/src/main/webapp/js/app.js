		...
		.state('tests',
		 	{
				...
				resolve: {
					testRecords: function(testHttp){
						return testHttp.getTestHistory();
					}
				}
			})
		.state('test',
		 	{
				...
				templateUrl: 'partials/testView/test.html',
				resolve: {
					test: function($stateParams, testHttp){
						return testHttp.getTest($stateParams.testId);
					}
				}
			})
		.state('test.diffs',
			{
				url: 'tests/:testId/diffs',
				controller: 'TestDiffsController',
				templateUrl: 'partials/testView/test.diffs.html',
				resolve:{
					testImages: function($stateParams, testHttp){
						return testHttp.getTestImages($stateParams.testId);
					}
				}	
			})
		.state('history',
...
app.run(['$rootScope', '$state', '$stateParams', '$http', 'configHttp', 'testHttp', 'historyHttp', 'socket', 
         function($rootScope, $state, $stateParams, $http, configHttp, testHttp, historyHttp, socket){
	...
	$rootScope.$on('$stateChangeSuccess', function(event, toState, fromState){
		...
		switch(toState.name){
			...
			case 'configCopy':
				...
				break;
			case 'tests':
				breadcrumb.links = [{text: 'Test History'}];
				sideNav.links = [{icon: 'icon-play', target: 'runTest', action: true, text: 'Run Test'}];
				break;
			case 'test':
				breadcrumb.links = [{text: 'Test Report'}];
				sideNav.links = [];
				break;
			case 'history':
