...
public class VipsUtils {
	...
	public static boolean USE_CDP = true;
	...
	public static String[] getVipsAttributes(){
		String[] returnArray  = {RECTANGLE, ISDISPLAYED, ISDIVIDABLE, ISVISUALBLOCK, ALREADYDIVIDED, PCOUNT,
...
//		}
	...
	public static Rectangle peelRectangleLayer(Rectangle rect, String toRemove) {
		...
		Rectangle returnRect= new Rectangle(rect.x + left, rect.y + top, rect.width -(left+right), rect.height-(top+bottom));
...
//	static String COMPUTEDSTYLESHEET_ALL = "return Array.from(document.querySelectorAll('*')).map(element => {return {webElement: element, computedStyle: getComputedStyle(element)}});";
	...
	public static Rectangle getContentRectangle(Node node, WebDriver driver) {
		...
		String javascriptReturn = ((JavascriptExecutor)driver).executeScript(CONTENT_RECTANGLE_JAVASCRIPT_FUNCTION + CONTENT_RECTANGLE_RETURN, vipsBlock).toString();
		LOG.debug(javascriptReturn);
		
		String[] split = javascriptReturn.split(":");
		if(split.length == 5) {
		Rectangle rect = new Rectangle(Integer.parseInt(split[0].trim()), Integer.parseInt(split[1].trim()),
