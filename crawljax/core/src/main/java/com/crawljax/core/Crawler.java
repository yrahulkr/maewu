...
//					if(previous.getId() != now.getId()) {
//						// cache the edge as form input disabled 
//						if(candidateActionCache.shouldDisableInput(action)) {
//							candidateActionCache.disableInputsForPath(event);
//						}
//					}
				}
...
//			} else {
			...
			// Just get the next best action  
			// Ignoring crawler Out of domain
...
//	private static void printCrawlPath(CrawlPath path) {
//		StringBuilder builder = new StringBuilder();
//		for(Eventable event: path) {
//			builder.append(event.getSourceStateVertex().getName());
//			builder.append("_");
