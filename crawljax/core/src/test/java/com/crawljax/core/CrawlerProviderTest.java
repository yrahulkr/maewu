...
public class CrawlerProviderTest {
	...
	private void setStateMachineForConfig(CrawljaxConfiguration config, InMemoryStateFlowGraph sfg) {
		sm = new StateMachine(sfg, config.getCrawlRules().getInvariants(),
				new Plugins(config, new MetricRegistry()), null, new ArrayList<>());
	}
	...
	public Crawler getCrawler(CrawljaxConfigurationBuilder configBuilder, EmbeddedBrowser browser) {
		...
		candidateActionCache = new UnfiredFragmentCandidates(config.getBrowserConfig(), sfgProvider, new MetricRegistry(), null);
		...
		Crawler crawler = new Crawler(context, config , stateComparator, candidateActionCache,
				formHandlerFactory, trainingFormHandlerFactory, waitConditionChecker, elementExtractorFactory, graphProvider,
				plugins, new HybridStateVertexFactory(0, configBuilder, false));
	...
	@Test
	public void test() {
//		String url = "http://localhost:8888/addressbook/addressbook-mod/addressbook/index.php";
		...
		configBuilder.crawlRules().click("div")
...
//		((HybridStateVertexImpl) sm.getCurrentState()).fragmentDom(browser.getWebDriver(), screenshot, screenshotFile);
	}
