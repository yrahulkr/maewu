...
public class StylePosition extends DomOperator{
...
//			Gson gson = new Gson();
			...
			record.setData(new MutationData("RandomLocation", point));
...
//			MutationData insertAttr = gson.fromJson(record.getData(), MutationData.class);
			...
			for(String style: styles) {
				...
				if(keyValue.length == 2 && 
						(keyValue[0].equalsIgnoreCase("position") || 
								keyValue[0].equalsIgnoreCase("left") ||
								keyValue[0].equalsIgnoreCase("top") )) {
					//ignore
				}
				else {
			...
			newStyleString += " ; position: absolute; left:"+ point.getX() + "px; top:" + point.getY() + "px";
...
//		Point 
		...
		sb.append("arguments[0].style.position = 'absolute';");
		sb.append("arguments[0].style.left = '" + point.getX() + "px';");
		sb.append("arguments[0].style.top = '" + point.getY() + "px';");
