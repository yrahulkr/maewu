...
if [[ $PRODUCTION == "yes"  ]]; then
	...
	docker run -it --workdir=/home/ppma --name=$CONTAINER_NAME --expose 80 --expose 3306 -p $PORT_APP:80 -p $PORT_DB:3306 -d --entrypoint ./run-services-docker.sh webappdockers/ppma:with052 bash
else
	if [[ $CONTAINER_NAME != "default" ]]; then
