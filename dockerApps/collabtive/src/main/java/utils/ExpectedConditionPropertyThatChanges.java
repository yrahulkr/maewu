...
//                System.out.println("ExpectedConditionPropertyThatChanges: element with locator " + this.locator + " not found!");
                return null; //no assumption about property changed
            }
        }
        else
            element = this.webElement;
        String currentAttributeValue = (String) js.executeScript("return arguments[0].getAttribute('" + this.attributeName + "')", element);
        //PageObjectLogging.logInfo("ExpectedConditionPropertyThatChanges. Attribute: " + attribute);
        if(currentAttributeValue != null && currentAttributeValue.equals(this.expectedValueAttribute)){
            return element;
        }
//        System.out.println("ExpectedConditionPropertyThatChanges: attribute " + currentAttributeValue + " != expected value " + this.expectedValueAttribute);
        return null;
    }
