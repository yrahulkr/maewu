...
if [[ $PRODUCTION == "yes"  ]]; then
	...
	#docker run -it --workdir=/home --name=$CONTAINER_NAME --expose 80 --expose 3306 -p $PORT_APP:80 -p $PORT_DB:3306 --entrypoint ./run-services-docker.sh  webappdockers/mantisbt:with121 bash
docker run -it --name=$CONTAINER_NAME --expose 80 --expose 3306 -p $PORT_APP:80 -p $PORT_DB:3306 -d webappdockers/mantisbt:with121 bash
